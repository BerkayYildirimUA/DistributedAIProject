from app.run_vehicle_pov import cls_names
import torch
import sys, os
...
class detected_classes_count:
    ...
    def make_mask(self, target_names, boxes):
        return torch.tensor(
    ...
    def count_objects(self, boxes):
        is_traffic_sign = self.make_mask({constants.OBJECT_CLASS_NAMES[4]}, boxes)
        is_pedestrian = self.make_mask({constants.OBJECT_CLASS_NAMES[5]}, boxes)
        is_traffic_light = self.make_mask({constants.OBJECT_CLASS_NAMES[3]}, boxes)
        is_vehicle_group = self.make_mask({constants.OBJECT_CLASS_NAMES[0], constants.OBJECT_CLASS_NAMES[1], constants.OBJECT_CLASS_NAMES[2]}, boxes)
