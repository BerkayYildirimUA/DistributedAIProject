...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            self.modify_vehicle_physics(self.player)
        # Set up the sensors.
        self.depth_camera_sensor = DepthCameraSensor(self.player, self.hud)
...
class DepthCameraSensor(object):
    def __init__(self, parent_actor, hud):
        ...
        camera_transform = carla.Transform(
            carla.Location(x=5.0, z=1),    # 2m forward, 1.5m above car
            carla.Rotation(pitch=0.0)
        )

        # Spawn the depth camera and attach to the vehicle
        self.sensor = world.spawn_actor(bp, camera_transform, attach_to=self._parent)

        # Avoid circular reference by using weakref
        weak_self = weakref.ref(self)
        self.sensor.listen(lambda image: DepthCameraSensor._on_image(weak_self, image))
    ...
    def _on_image(weak_self, image):
        ...
        # Convert to depth in meters (CARLA uses 1000m max range)
        depth_meters = normalized_depth * 1000.0

        # Clip depth to max of 50 meters for visualization
        depth_vis = np.clip(depth_meters, 0, 50)

        # Normalize to 0–255 for OpenCV display
        depth_vis_uint8 = (depth_vis / 50.0 * 255).astype(np.uint8)
