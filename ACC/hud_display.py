...
class HUD(object):
    ...
    def __init__(self, width, height):
        ...
        # Base font for notifications
        font = pygame.font.Font(pygame.font.get_default_font(), 20)

        # Try to get a monospace font similar to CARLA examples
        try:
            font_name = 'courier' if os.name == 'nt' else 'mono'
            fonts = [x for x in pygame.font.get_fonts() if font_name in x]
            default_font = 'ubuntumono'
            mono_name = default_font if default_font in fonts else fonts[0]
            mono = pygame.font.match_font(mono_name)
        except Exception:
            # Fallback to default font
            mono = pygame.font.get_default_font()

        # Monospace font used to render text rows
        self._font_mono = pygame.font.Font(mono, 12 if os.name == 'nt' else 14)

        # Notification and help systems
    ...
    def on_world_tick(self, timestamp):
        """Gets information from the world at every tick (server-side)"""
    ...
    def tick(self, world, ego, clock):
        ...
        self._info_text = [
            ...
            '',
            'Vehicle: % 20s' % get_actor_display_name(ego, truncate=20),
        ...
        vehicles = [(dist(x.get_location()), x) for x in vehicles if x.id != ego.id]
